...
def generate_latex_pdf(analysis: Dict[str, Any], runs_data: List[List[float]], temp_dir: str) -> bytes:
    ...
    latex_content = generate_latex_content(analysis, runs_data, temp_dir)
...
def generate_latex_content(analysis: Dict[str, Any], runs_data: List[List[float]], temp_dir: str) -> str:
    ...
    if "ecdf_all_runs" in analysis:
        ecdf_path = generate_overlaid_ecdf_chart(analysis, temp_dir)
        if ecdf_path:
    ...
    if "individual_analyses" in analysis:
        qq_path = generate_overlaid_qq_chart(analysis, temp_dir)
        if qq_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
    ...
    if "individual_analyses" in analysis:
        for run_idx, run_analysis in enumerate(analysis["individual_analyses"]):
            ...
            latex += generate_per_run_analysis(run_analysis, run_idx + 1, temp_dir)
...
def generate_overlaid_ecdf_chart(analysis: Dict[str, Any], temp_dir: str) -> str:
...
def generate_overlaid_qq_chart(analysis: Dict[str, Any], temp_dir: str) -> str:
...
def generate_per_run_analysis(run_analysis: Dict[str, Any], run_num: int, temp_dir: str) -> str:
    ...
    basic_stats = run_analysis.get("basic_stats", {})
    latex += generate_descriptive_stats_table(basic_stats)
    ...
    # KDE chart
    if "kde" in dist:
        kde_path = generate_kde_chart(dist["kde"], run_num, temp_dir)
        if kde_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
    ...
    if "range_behavior" in run_analysis and "ecdf" in run_analysis["range_behavior"]:
        ecdf_path = generate_ecdf_chart(run_analysis["range_behavior"]["ecdf"], run_num, temp_dir)
        if ecdf_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
            latex += f"\\includegraphics[width=0.8\\textwidth]{{{ecdf_path}}}" + "\n"
    ...
    # Q-Q Plot
    if "qq_plot" in dist:
        qq_path = generate_qq_chart(dist["qq_plot"], run_num, temp_dir)
        if qq_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
            latex += f"\\includegraphics[width=0.8\\textwidth]{{{qq_path}}}" + "\n"
    ...
    if "autocorrelation" in ind:
        acf_path = generate_acf_chart(ind["autocorrelation"], run_num, temp_dir)
        if acf_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
    ...
    if "lag1_scatter" in ind:
        lag1_path = generate_lag1_scatter_chart(ind["lag1_scatter"], run_num, temp_dir)
        if lag1_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
    ...
    if "time_series" in ind:
        ts_path = generate_time_series_chart(ind["time_series"], run_num, temp_dir)
        if ts_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
    ...
    if "rolling_mean" in stat and "rolling_std" in stat:
        rolling_path = generate_rolling_stats_chart(stat, run_num, temp_dir)
        if rolling_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
    ...
    if "frequencies" in spec and "magnitude" in spec:
        fft_path = generate_fft_chart(spec, run_num, temp_dir)
        if fft_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
    ...
    if "frequencies" in spec and "power" in spec:
        power_path = generate_power_spectrum_chart(spec, run_num, temp_dir)
        if power_path:
            latex += r"\begin{figure}[H]" + "\n"
            latex += r"\centering" + "\n"
    ...
    if nist_tables:
        latex += generate_nist_tables_grid(nist_tables)
    
    return latex


def generate_descriptive_stats_table(basic_stats: Dict[str, Any]) -> str:
...
def generate_kde_chart(kde_data: Dict[str, Any], run_num: int, temp_dir: str) -> str:
    """Generate KDE chart"""
...
def generate_ecdf_chart(ecdf_data: Dict[str, Any], run_num: int, temp_dir: str) -> str:
    """Generate ECDF chart"""
...
def generate_qq_chart(qq_data: Dict[str, Any], run_num: int, temp_dir: str) -> str:
    """Generate Q-Q plot"""
...
def generate_acf_chart(acf_data: Dict[str, Any], run_num: int, temp_dir: str) -> str:
    """Generate ACF chart"""
...
def generate_lag1_scatter_chart(lag1_data: Dict[str, Any], run_num: int, temp_dir: str) -> str:
...
def generate_time_series_chart(ts_data: Dict[str, Any], run_num: int, temp_dir: str) -> str:
...
def generate_rolling_stats_chart(stat_data: Dict[str, Any], run_num: int, temp_dir: str) -> str:
...
def generate_fft_chart(spec_data: Dict[str, Any], run_num: int, temp_dir: str) -> str:
...
def generate_power_spectrum_chart(spec_data: Dict[str, Any], run_num: int, temp_dir: str) -> str:
...
def generate_nist_test_table(nist_test: Dict[str, Any], test_name: str) -> str:
    """Generate LaTeX table for NIST test results (standalone version)"""
    if "error" in nist_test and nist_test["error"]:
        return f"\\textbf{{{test_name}:}} Error: {nist_test['error']}\\\\[1em]\n\n"
    
    latex = r"\begin{table}[H]" + "\n"
    latex += r"\centering" + "\n"
    latex += generate_nist_test_table_content(nist_test, test_name)
    latex += r"\end{table}" + "\n\n"
